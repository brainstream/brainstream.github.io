...
> Все примеры в этой статье компилируются и запускаются в операционной системе на базе Linux. В других операционных системах детали могут отличаться.
...
Пространства имён (namespace) без имени называются безымянными (unnamed) или анонимными (anonymous). Давайте попробуем разобраться, что это и для чего они нужны на практике.
...
Команда `nm` не вывела никаких результатов. Метка `static` у функции просит компилятор не выставлять наружу соответствующий символ. Значит связывание могло произойти только с реализаций из модуля `module1.cpp`. В этом легко убедиться, модифицировав код в модулях следующим образом:
